...
class AuthorizeController(http.Controller):
    _return_url = '/payment/alignet/return/'
    _cancel_url = '/payment/alignet/cancel/'

    @http.route([
        '/payment/alignet/return/',
        '/payment/alignet/cancel/',
    ], type='http', auth='public', csrf=False)
    def authorize_form_feedback(self, **post):
        _logger.info('VPOS ALIGNET: entering form_feedback with post data %s', pprint.pformat(post))
        return_url = '/'
        if post:
